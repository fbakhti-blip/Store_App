...
class WarehouseRepository:
    ...
    def save(self, warehouse):
        self.connect()
        self.cursor.execute("""insert into warehouses (product_id,quantity) values (?,?)""",
    ...
    def update(self, warehouse):
        self.connect()
        self.cursor.execute("""update warehouses set product_id=?,quantity=? where id=?""",
                            ([warehouse.product_id, warehouse.quantity, warehouse.warehouse_id]))
        self.connection.commit()
        self.disconnect()
        return warehouse
